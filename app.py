...
with tab5:
    ...
    if predict_btn:
        ...
        # Show how this customer compares to segment average
        st.markdown("### 📊 How This Customer Compares to Segment Average")
        seg_avg = df[df['Segment'] == segment][FEATURES].mean()

        compare_data = pd.DataFrame({
            'Feature':        FEATURES,
            'This Customer':  input_data[0],
            'Segment Average': seg_avg.values
        })

        fig_compare = go.Figure()
        fig_compare.add_trace(go.Bar(
            name='This Customer',
            x=compare_data['Feature'],
            y=compare_data['This Customer'],
            marker_color=color, opacity=0.9
        ))
        fig_compare.add_trace(go.Bar(
            name='Segment Average',
            x=compare_data['Feature'],
            y=compare_data['Segment Average'],
